...
import com.cappielloantonio.tempo.util.Preferences.setStarredArtistsSyncEnabled
import com.cappielloantonio.tempo.viewmodel.StarredArtistsSyncViewModel
import com.google.android.material.dialog.MaterialAlertDialogBuilder
import java.util.stream.Collectors

@OptIn(markerClass = [UnstableApi::class])
class StarredArtistSyncDialog(private val onCancel: Runnable?) : DialogFragment() {
    private var starredArtistsSyncViewModel: StarredArtistsSyncViewModel? = null

    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val bind = DialogStarredArtistSyncBinding.inflate(getLayoutInflater())

        starredArtistsSyncViewModel =
            ViewModelProvider(requireActivity()).get<StarredArtistsSyncViewModel>(
                StarredArtistsSyncViewModel::class.java
            )
    ...
    private fun setButtonAction(context: Context?) {
        ...
        if (dialog != null) {
            ...
            positiveButton.setOnClickListener(View.OnClickListener { v: View? ->
                starredArtistsSyncViewModel!!.getStarredArtistSongs(requireActivity())
